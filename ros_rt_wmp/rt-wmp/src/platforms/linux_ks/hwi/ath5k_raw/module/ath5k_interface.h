...
/* RUBEN */
/*
 * Exported symbols for using with RT-WMP
 */
#ifdef __KERNEL__
/**
 *  'netdev' will point to the ath5k_raw interface that must be used by RT-WMP
 */
extern void get_ath5k_slave_dev(struct net_device**);
 ...
 */
extern int ath5k_rx(struct sk_buff **);
 ...
 *  If no packet has been dequeued, the return value is 0.
 */
extern int ath5k_rx_timeout(struct sk_buff **, int);

/**
 *  Unlocks the reception semaphore
 */
extern void unlock_rx_sem(void);
 ...
 */
extern int conf_ath5k (struct net_device *, union ath5k_ioctl_info *, int);
#endif
