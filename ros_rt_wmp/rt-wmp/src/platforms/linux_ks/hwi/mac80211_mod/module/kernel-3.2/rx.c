		     ...
		     struct ieee80211_rate *rate)
{
	...
	if (rt_wmp_is_active()){
		...
		skb3 = remove_monitor_info(local, skb3);
		ieee80211_add_rx_radiotap_header(local, skb3, rate, needed_headroom);
		//skb3->dev = sdata->dev;
		//printk(KERN_ERR "DEV:%p\n", prev_dev);
		rt_wmp_netif_receive_skb(skb3,status->signal);
	}
	/* DANILO */
...
void ieee80211_rx(struct ieee80211_hw *hw, struct sk_buff *skb)
{
	...
	//printk(KERN_ERR "quattro\n");


	if (WARN_ON(!local->started))
		goto drop;

	//printk(KERN_ERR "Cinque\n");
