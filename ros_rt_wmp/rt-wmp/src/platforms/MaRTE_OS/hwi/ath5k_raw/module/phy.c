		...
		struct ieee80211_channel *channel)
{
	...
	unsigned int ath5k_channel =
		ath5k_frequency_to_channel(channel->center_freq);
...
ath5k_hw_calibration_poll(struct ath5k_hw *ah)
{
	...
	if (timespec_to_double(&ah->ah_cal_tstamp) == 0)
		ah->ah_cal_tstamp = actual;

	siguiente = ah->ah_cal_tstamp;
	incr_timespec(&siguiente, &ah->ah_cal_intval);

	if (smaller_timespec(&siguiente, &actual))
	{
		ah->ah_cal_tstamp = actual;
