...
char * wmp_get_frame_routing_pointer_new_tree(wmpFrame * t){
	char * tkn_data_ptr =(char*)t;
	tkn_data_ptr+=(sizeof(Token_Hdr)+sizeof(Token)+2*status.N_NODES);
	return tkn_data_ptr;
}

char * wmp_get_frame_routing_pointer_status(wmpFrame * t){
	char * tkn_data_ptr =(char*)t;
	tkn_data_ptr+=(sizeof(Token_Hdr)+sizeof(Token)+status.N_NODES);
	return tkn_data_ptr;
}
...
char * wmp_get_message_data_pointer(wmpFrame * t){
	...
	tkn_data_ptr+=(sizeof(Token_Hdr)+sizeof(Message)+status.N_NODES);
	return tkn_data_ptr;
}
...
int wmp_get_frame_total_lenght(wmpFrame * t){
	...
	if (t->hdr.type==TOKEN){
		size+=sizeof(Token) + 3*status.N_NODES;
	} else if (t->hdr.type==AUTHORIZATION){
		size+=sizeof(Authorization) + status.N_NODES;
	} else if (t->hdr.type==MESSAGE){
		size+=sizeof(Message) + status.N_NODES + t->msg.len;
