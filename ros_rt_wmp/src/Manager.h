...
protected:
	...
	enum wmp_commands_t {
		STOP, TOPIC_STOP, TOPIC_START, TOPIC_DECIMATE, TOPIC_JUSTONE, SET_PRIORITY, GET_PRIORITY, RECONNECT, SET_FAKE_LQM, SET_PREDEFINED_FAKE_LQM, SET_FAKE_PATH, SET_WMP_PARAM, GET_WMP_PARAM
	};
public:
	ros::NodeHandle * n;
	...
	template <typename P> int serialize(char * p, const boost::shared_ptr<P const> & pm){
		...
		if (res == Z_OK){
			ROSWMP_DEBUG(stderr,"After compressing %s: %d\n",name.c_str(),zsize);
			return zsize;
		}
